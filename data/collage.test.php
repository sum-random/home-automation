...
<script type="text/javascript" src="/jscript/collage.test.js"></script>
</HEAD>
<BODY>
...
function insertCell(&$list, $offset, $colors){
  ...
  $imgsrc="/wsgi-bin/thumbnail?IMGID=" . $subimgid;
  $list[$offset]="<TD STYLE='background-color: #" . $bgcolor . ";' bgcolor='" . $bgcolor . "'><A TITLE='" . $bgcolor . "' HREF='/collage.test.php?IMGID=" . $subimgid . "&THUMBSZ=" . $thumbsz . "&SCALE=" . $scaleimg . "' BORDER='0' ><IMG ID='IMGCELL' onMouseOver='showPreview();' onMouseOut='hidePreview();' BORDER='0' ALT='" . $imgquery . "' SRC='" . $imgsrc . "&SIZE=" . $thumbsz . "x" . $thumbsz . "' HEIGHT='" . $thumbsz . "' WIDTH='" . $thumbsz . "'></A></TD>\n";
...
if(array_key_exists('IMGID',$_REQUEST)) {
  ...
  exit(1);
      ...
      echo "<INPUT TYPE='IMAGE' NAME='IMG[" . $srcimgid . "]' SRC='/wsgi-bin/thumbnail?IMGID=" . $srcimgid . "&SIZE=512'>";
      echo "</FORM></TH></TR>\n";
      for($y=0; $y<$height; $y++) {
        echo "<TR>\n";
        for($x=1; $x<$width; $x++) {
